...
#    if defined(SC32f12xx) || defined(SC32R805) || defined(SC32R806) || defined(SC32R803)
    ...
    /* Clear OPFS bits */
    tmpreg &= (uint32_t)~(OP_CON_OPOSEL | OP_CON_OPNSEL | OP_CON_OPPSEL | OP_CON_PGAGAIN |
                          OP_CON_FDBRSEL | OP_CON_PGAOFC);
    /* Set OPFS bit to OP_FreqSelect value */
    tmpreg |= (uint32_t)(OP_InitStruct->OP_FDBResisrance | OP_InitStruct->OP_Negative |
                         OP_InitStruct->OP_Output | OP_InitStruct->OP_PGAGain |
                         OP_InitStruct->OP_Posittive | OP_InitStruct->OP_ShortCircuit);
...
#    elif defined(SC32f15xx)
    ...
    if (OPx == OP_0) {
        ...
        OP->OPX_CFG |= (OP_InitStruct->OP_CMPIM1 << OP_CFG_OPCMPIM1_Pos) |
                       (OP_InitStruct->OP_REFSEL << OP_CFG_REFSEL_Pos);
        ...
        OP->OPX_CFG |= (OP_InitStruct->OP_CMPIM2 << OP_CFG_OPCMPIM2_Pos) |
                       (OP_InitStruct->OP_REFSEL << OP_CFG_REFSEL_Pos);
    }
#    endif
}
...
#    if defined(SC32f12xx) || defined(SC32R805) || defined(SC32R806) || defined(SC32R803)
    ...
    tmpreg |= (uint32_t)((OP_TrimValueL << OP_CON_TRIMOFFSETP_Pos) |
                         (OP_TrimValueH << OP_CON_TRIMOFFSETN_Pos));
 ...
 * times
 *                  - OP_PGAGain_NonInvert8_Invert7:The OP inverting input gain is 8/7
 * times
 *                  - OP_PGAGain_NonInvert16_Invert15:The OP inverting input gain is 16/15
 * times
 *                  - OP_PGAGain_NonInvert32_Invert31:The OP in-phase input gain is 32/31
 * times
 *                  - OP_PGAGain_NonInvert64_Invert63:The OP in-phase input gain is 64/63
 * times
 * @retval None
 */
...
#    elif defined(SC32f15xx)
    ...
    if (OPx == OP_0) {
        OP->OP0_CON &= ~OP_CON_PGAGAIN;
        OP->OP0_CON |= PGAGain;
    } else if (OPx == OP_1) {
        OP->OP1_CON &= ~OP_CON_PGAGAIN;
        OP->OP1_CON |= PGAGain;
    } else if (OPx == OP_2) {
        OP->OP2_CON &= ~OP_CON_PGAGAIN;
        OP->OP2_CON |= PGAGain;
    }
#    endif
}
...
ErrorStatus OP_OffsetSet(OP_TypeDef * OPx) {
    ...
    OPx_offsetchecksum0 =
        (*((uint8_t *)0X08C00000 + 0x480)) + (*((uint8_t *)0X08C00000 + 0x481)) +
        (*((uint8_t *)0X08C00000 + 0x482)) + (*((uint8_t *)0X08C00000 + 0x483));
    OPx_offsetchecksum1 =
        ((*((uint8_t *)0X08C00000 + 0x484)) << 8) | (*((uint8_t *)0X08C00000 + 0x485));
    if ((OPx_Readoffset0 == 0x55) && (OPx_Readoffset2 == 0x55) &&
        (OPx_offsetchecksum0 == OPx_offsetchecksum1)) {
        uint32_t tmpreg = 0;
        /*!< Get the OPAMPx_CSR register value */
        tmpreg = OPx->OP_CON;
        ...
        /*!< Configure the new trimming value */
        tmpreg |= (uint32_t)((OPx_Readoffset3 << OP_CON_TRIMOFFSETN_Pos) |
                             (OPx_Readoffset1 << OP_CON_TRIMOFFSETP_Pos));
 ...
 *               -OP_TriggerMode_RISE:The trigger mode of the simulated comparator is
 * rising edge -OP_TriggerMode_FALL:The trigger mode of simulated comparator is falling
 * edge -OP_TriggerMode_RISE_FALL:The trigger mode of the simulated comparator is rising
...
void OP_OffsetZero(OP_TypeDef * OPx) {
    ...
    if (OPx == OP_0) {
        ...
        /*!< Configure the new trimming value */
        tmpreg |= (uint32_t)((OP_Nmos_offset << OP_CON_TRIMOFFSETN_Pos) |
                             (OP_Pmos_offset << OP_CON_TRIMOFFSETP_Pos));
 ...
 *               - OP_CMPSTA_Low: The non-inverting input is at a lower voltage than the
 * inverting input
 *               - OP_CMPSTA_High: The non-inverting input is at a higher voltage than the
 * inverting input
 */
