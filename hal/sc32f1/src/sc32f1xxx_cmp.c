 ...
 *               - CMP_2: select the CMP2 peripheral
 *               - CMP3: select the CMP3 peripheral
 * @param  CMP_InitStruct[out]:pointer to a CMP_InitTypeDef structure which will be
 * initialized.
 * @retval None
 */
void CMP_Init(CMP_TypeDef * CMPx, CMP_InitTypeDef * CMP_InitStruct) {
    ...
    tmpreg = CMPx->CMP_CFG;
    /* Clear CMPFS bits */
    tmpreg &= (uint32_t)~(CMP_CFG_CMPIS | CMP_CFG_CMPP | CMP_CFG_CMPIM | CMP_CFG_CMPRF);
    ...
    if (CMPx == CMP_0 || CMPx == CMP_1 || CMPx == CMP_2) {
        ...
        if (CMPx == CMP_0) {
            ...
            /* Set CMPIM bits according to CMP_TriggerMode value */
            tmpreg |=
                (uint32_t)(CMP_InitStruct->CMP_Negative | CMP_InitStruct->CMP_Positive |
                           CMP_InitStruct->CMP_TriggerMode);
            /* Write to CMPx CMP_CFG */
            CMP->CMP_CFG = tmpreg;
        } else if (CMPx == CMP_1) {
        ...
        tmpreg &=
            (uint16_t)~(CMP_CFG_CMPIM | CMP_CFG_CMPNS | CMP_CFG_CMPPS | CMP_CFG_CMPRF);
 ...
 *                  - CMP_Positive_1_5V:Select CMPP as the CMP input port,CMPP is 1.5V
 * reference voltage
 ...
 *                  - CMP0_1_2NegativeSelect_CMPxN:Select CMPxN as the CMP0_1_2 Negative
 * port
 *                  - CMP0_1_2NegativeSelect_DAC:Select DAC as the CMP0_1_2 Negative port
 *                  - CMP0_1_2NegativeSelect_BEMF_MID:Select BEMF_MID as the CMP0_1_2
 * Negative port
 ...
 *                  - CMP3_NegativeSelect_CMPRF:Select CMPRF as the CMP negative input
 * port
 * @retval None
 */
void CMP_NegativeSelection(CMP_TypeDef *              CMPx,
                           CMP_NegativeSelect_TypeDef NegativeSelect) {
    /* Check the parameters */
    assert_param(IS_CMP_ALL_PERIPH(CMPx));
 ...
 *               - CMP_CMPSTA_Low:The non-inverting input is at a lower voltage than the
 * inverting input
 *               - CMP_CMPSTA_High: The non-inverting input is at a higher voltage than
 * the inverting input
 *               - CMP_CMP0STA_Low: The non-inverting input is at a lower voltage than the
 * inverting input
 *               - CMP_CMP0STA_High: The non-inverting input is at a higher voltage than
 * the inverting input
 *               - CMP_CMP1STA_Low	: The non-inverting input is at a lower voltage than
 * the inverting input
 *               - CMP_CMP1STA_High: The non-inverting input is at a higher voltage than
 * the inverting input
 *               - CMP_CMP2STA_Low: The non-inverting input is at a lower voltage than the
 * inverting input
 *               - CMP_CMP2STA_High: The non-inverting input is at a higher voltage than
 * the inverting input
 *               - CMP_CMP3STA_Low: CMP3 positive terminal voltage is less than the
 * negative terminal voltage
 *               - CMP_CMP3STA_High:The positive terminal voltage of CMP3 is greater than
