...
void SPI1_Init(SPITWI_TypeDef * SPIx, SPI1_InitTypeDef * SPI1_InitStruct) {
    ...
    if (SPIx == SPITWI_SPI1) {
        ...
        tmpreg |=
            (uint32_t)(SPI1_InitStruct->SPI1_Mode | SPI1_InitStruct->SPI1_DataSize |
...
void TWI1_Init(SPITWI_TypeDef * TWIx, TWI1_InitTypeDef * TWI_InitStruct) {
    ...
    if (TWIx == SPITWI_TWI1) {
        ...
        tmpreg &= (uint32_t)~(TWI1_CON_AA | SPI1_TWI1_CON_QTWCK_Msk | TWI1_CON_STRETCH |
                              SPI1_TWI1_CON_MODE_Msk);
        ...
        tmpreg |= (uint32_t)(TWI_InitStruct->TWI1_GeneralCall |
                             (TWI_InitStruct->TWI1_SlaveAdress & 0xFE));
...
uint8_t TWI1_ReceiveData(SPITWI_TypeDef * TWIx) {
    ...
    return (uint8_t)TWIx->SPI1_TWI1_DATA;
}
...
void TWI1_ClearFlag(SPITWI_TypeDef * TWIx, TWI1_FLAG_TypeDef TWI1_FLAG) {
    ...
    if (TWIx == SPITWI_TWI1) {
        /* Clear the flags */
        TWIx->SPI1_TWI1_STS |= (uint16_t)TWI1_FLAG;
    }
}
