...
/** @defgroup PGA_Exported_Functions_Group1 Configuration of the PGA computation unit
functions
...
void PGA_Init(PGA_TypeDef * PGAx, PGA_InitTypeDef * PGA_InitStruct) {
    ...
    /* Clear PGAFS bits */
    tmpreg &=
        (uint32_t)~(PGA_CON_PGACOM | PGA_CON_PGAGAN | PGA_CON_PGAIPT | PGA_CON_PGAINSEL);
    /* Set PGAFS bit to PGA_FreqSelect value */
