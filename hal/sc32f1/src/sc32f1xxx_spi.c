...
void SPI_Init(SPI_TypeDef * SPIx, SPI_InitTypeDef * SPI_InitStruct) {
    ...
    tmpreg &= (uint32_t)~(SPI_CON_MSTR | SPI_CON_SPMD | SPI_CON_DORD | SPI_CON_CPHA |
                          SPI_CON_CPOL | SPI_CON_SPR);
    /* Configure SPIx: mode, data size, first transmitted bit,clock predivision , CPOL and
     * CPHA */
    ...
    if (SPIx == SPI0) {
        ...
        /* Configure SPIx: mode, data size, first transmitted bit,clock predivision , CPOL
         * and CPHA
         */
    ...
    if (SPIx == SPI1) {
        ...
        tmpreg |= (uint32_t)(((SPI_InitStruct->SPI_Mode | SPI_InitStruct->SPI_DataSize |
                             ...
                             << 16) |
                  (0x01 << SPI1_TWI1_CON_MODE_Pos) | SPI_InitStruct->SPI_Prescaler;
    ...
    tmpreg = SPIx->SPI_CON;
    /* Clear MSTR, SPMD, DORD, CPOL, SPR bits */
    tmpreg &= (uint32_t)~(TWI_QSPIx_CON_SMSTR | TWI_QSPIx_CON_DWIDTH |
                          TWI_QSPIx_CON_DORD | TWI_QSPIx_CON_CPHA | TWI_QSPIx_CON_CPOL |
                          TWI_QSPIx_CON_QTWCK | TWI_QSPIx_CON_MODE);
    /* Configure SPIx: mode, data size, first transmitted bit,clock predivision , CPOL and
     * CPHA */
    /* Set MSTR bits to SPI0_Mode value */
    /* Set SPMD bit according to SPI1_DataSize value */
    ...
    tmpreg |= (uint32_t)(SPI_InitStruct->SPI_Mode | SPI_InitStruct->SPI_DataSize |
                         SPI_InitStruct->SPI_FirstBit | SPI_InitStruct->SPI_CPHA |
                         SPI_InitStruct->SPI_CPOL) |
              (0x01 << TWI_QSPIx_CON_MODE_Pos) | SPI_InitStruct->SPI_Prescaler;
...
void SPI_Cmd(SPI_TypeDef * SPIx, FunctionalState NewState) {
    ...
    if (NewState != DISABLE) {
        /* Enable the SPI TX Function */
        SPIx->SPI_CON |= TWI_QSPIx_CON_QTWEN;
    } else {
        /* Disable the SPI TX Function */
        SPIx->SPI_CON &= (uint16_t)~TWI_QSPIx_CON_QTWEN;
    }
#endif
}
 ...
 *               - SPI_DataSize_32B:Set data frame format to 32bit
 * @retval None
 */
void SPI_DataSizeConfig(SPI_TypeDef * SPIx, SPI_DataSize_TypeDef SPI_DataSize) {
    ...
    /* Clear SPMD bit */
    SPIx->SPI_CON &= (uint16_t)~TWI_QSPIx_CON_DWIDTH;
    /* Set new SPMD bit value */
    SPIx->SPI_CON |= SPI_DataSize;
#endif
}
...
void SPI_SendDataFIFO(SPI_TypeDef * SPIx, uint32_t * Data, uint16_t length) {
    ...
    if (SPIx->SPI_CON & SPI_CON_SPMD) {
        for (tmpNum = 0; tmpNum < length; tmpNum++) {
            SPIx->SPI_DATA = (uint16_t)(*((uint16_t *)Data + tmpNum));
        }
    }
    /* Transmit 8 Bit Data */
    else {
    ...
    if ((SPIx == SPI0) || (SPIx == SPI1)) {
        if ((SPIx->SPI_CON & TWI_QSPIx_CON_DWIDTH) >= SPI_DataSize_24B) {
            for (tmpNum = 0; tmpNum < length; tmpNum++) {
                SPIx->SPI_DATA = (uint32_t)(*((uint32_t *)Data + tmpNum));
            }
        } else if ((SPIx->SPI_CON & TWI_QSPIx_CON_DWIDTH) == SPI_DataSize_16B) {
            for (tmpNum = 0; tmpNum < length; tmpNum++) {
                SPIx->SPI_DATA = (uint32_t)(*((uint16_t *)Data + tmpNum));
            }
        }
        /* Transmit 8 Bit Data */
        else if ((SPIx->SPI_CON & TWI_QSPIx_CON_DWIDTH) == SPI_DataSize_8B) {
            for (tmpNum = 0; tmpNum < length; tmpNum++) {
                SPIx->SPI_DATA = (uint32_t)(*((uint8_t *)Data + tmpNum));
            }
        }
    }
#endif
...
void SPI_ReceiveDataFIFO(SPI_TypeDef * SPIx, uint32_t * Data, uint16_t length) {
    ...
    if (SPIx->SPI_CON & SPI_CON_SPMD) {
        for (tmpNum = 0; tmpNum < length; tmpNum++) {
            (*((uint16_t *)Data + tmpNum)) = (uint16_t)SPIx->SPI_DATA;
        }
    }
    /* Transmit 8 Bit Data */
    else {
    ...
    if ((SPIx == SPI0) || (SPIx == SPI1)) {
        if ((SPIx->SPI_CON & TWI_QSPIx_CON_DWIDTH) >= SPI_DataSize_24B) {
            for (tmpNum = 0; tmpNum < length; tmpNum++) {
                (*((uint32_t *)Data + tmpNum)) = (uint32_t)SPIx->SPI_DATA;
            }
        } else if ((SPIx->SPI_CON & TWI_QSPIx_CON_DWIDTH) == SPI_DataSize_16B) {
            for (tmpNum = 0; tmpNum < length; tmpNum++) {
                (*((uint16_t *)Data + tmpNum)) = (uint16_t)SPIx->SPI_DATA;
 ...
 *            - SPI_PinRemap_C  TIM Pin Remap select Remap mode C
 * @retval None
 */
 ...
 *               - SPI_IT_TBIE  SPI Interrupt: Transmit TBIE under half Interrupt
