...
/** @defgroup QEP_Exported_Functions_Group1 Configuration of the QEP computation unit
functions
...
void QEP_Init(QEP_TypeDef * QEPx, QEP_InitTypeDef * QEP_InitStruct) {
    ...
    tmpreg |=
        (QEP_InitStruct->QEP_Mode | QEP_InitStruct->QEP_CEN | QEP_InitStruct->QEP_GATE |
         QEP_InitStruct->QEP_QFDIV | QEP_InitStruct->QEP_IndexMode |
         QEP_InitStruct->QEP_QEPnI | QEP_InitStruct->QEP_SWAP);

    tmpreg |= (QEP_InitStruct->QEP_QAP << QEP_CON_QAP_Pos) |
              (QEP_InitStruct->QEP_QBP << QEP_CON_QBP_Pos);
 ...
 * @param  QEP_InitStruct[out]: pointer to a QEP_InitTypeDef structure which will be
 * initialized.
 * @retval None
 */
