...
void TWI_Init(TWI_TypeDef * TWIx, TWI_InitTypeDef * TWI_InitStruct) {
    ...
    tmpreg |= (uint32_t)(TWI_InitStruct->TWI_GeneralCall |
                         ((TWI_InitStruct->TWI_SlaveAdress << 1) & 0xFE));
    ...
    tmpreg = TWIx->TWI_CON;
    /* Clear AA, TWCK, STRETCH bits */
    tmpreg &= (uint32_t)~(TWI_QSPIx_CON_AA | TWI_QSPIx_CON_SPOS | TWI_QSPIx_CON_STRETCH |
                          TWI_QSPIx_CON_QTWCK);
    /* Configure TWIx: Ack, Clock division and Slave Clock Stretch */
    /* Set AA bits to TWI_Ack value */
    /* Set TWCK bits according to TWI_DataSize value */
...
void TWI_Cmd(TWI_TypeDef * TWIx, FunctionalState NewState) {
    ...
    if (NewState != DISABLE) {
        /* Enable the TWI Function */
        TWIx->TWI_CON |= TWI_SPIx_CON_QTWEN;
    } else {
        /* Disable the TWI Function */
        TWIx->TWI_CON &= (uint16_t)~TWI_SPIx_CON_QTWEN;
    }
#endif
}
...
void TWI_AcknowledgeConfig(TWI_TypeDef * TWIx, FunctionalState NewState) {
    ...
    if (NewState != DISABLE) {
        /* Enable the acknowledgement */
        TWIx->TWI_CON |= TWI_SPIx_CON_AA;
    } else {
        /* Disable the acknowledgement */
        TWIx->TWI_CON &= (uint16_t)~TWI_SPIx_CON_AA;
    }
#endif
}
...
void TWI_GeneralCallCmd(TWI_TypeDef * TWIx, FunctionalState NewState) {
    ...
    if (NewState != DISABLE) {
        /* Enable general call */
        TWIx->TWI_ADD |= TWI_QSPIx_ADD_GC;
    } else {
        /* Disable general call */
        TWIx->TWI_ADD &= (uint16_t)~TWI_QSPIx_ADD_GC;
    }
#endif
}
...
void TWI_StretchClockConfig(TWI_TypeDef * TWIx, FunctionalState NewState) {
    ...
    if (NewState == DISABLE) {
        /* Enable the selected TWI Clock stretching */
        TWIx->TWI_CON |= TWI_QSPIx_CON_STRETCH;
    } else {
        /* Disable the selected TWI Clock stretching */
        TWIx->TWI_CON &= (uint16_t)~((uint16_t)TWI_QSPIx_CON_STRETCH);
    }
#endif
}
...
void TWI_SetNbytes(TWI_TypeDef * TWIx, uint8_t Nbytes) {
    ...
    TWIx->TWI_STS = (uint32_t)(Nbytes << TWI_QSPIx_STS_NBYTES_Pos);
#endif
}
...
uint8_t TWI_GetNbytes(TWI_TypeDef * TWIx) {
    ...
    tmpnum = (uint8_t)(TWIx->TWI_STS >> TWI_QSPIx_STS_NBYTES_Pos);
...
void TWI_GenerateSTART(TWI_TypeDef * TWIx, FunctionalState NewState) {
    ...
    if (NewState != DISABLE) {
        /* Generate a START condition */
        TWIx->TWI_CON |= TWI_SPIx_CON_START;
    } else {
        /* Disable the START condition generation */
        TWIx->TWI_CON &= (uint16_t)~TWI_SPIx_CON_START;
    }
#endif
}
...
void TWI_GenerateSTOP(TWI_TypeDef * TWIx, FunctionalState NewState) {
    ...
    if (NewState != DISABLE) {
        /* Generate a STOP condition */
        TWIx->TWI_CON |= TWI_SPIx_CON_STOP;
    } else {
        /* Disable the STOP condition generation */
        TWIx->TWI_CON &= (uint16_t)~TWI_SPIx_CON_STOP;
    }
#endif
}
 ...
 * @param  TWI_Command[in]:specifies whether the TWI device will be a Transmitter or a
 * Receiver.
...
TWI_StateMachine_TypeDef TWI_GetStateMachine(TWI_TypeDef * TWIx) {
    ...
    return (TWI_StateMachine_TypeDef)(TWIx->TWI_STS & TWI_SPIx_STS_STATE);
#endif
}
