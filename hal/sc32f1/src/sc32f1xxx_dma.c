...
void DMA_Init(DMA_TypeDef * DMAx, DMA_InitTypeDef * DMA_InitStruct) {
    ...
    tmpreg &= (uint32_t)~(DMA_CFG_PL | DMA_CFG_TXWIDTH | DMA_CFG_CIRC | DMA_CFG_CHRST |
                          DMA_CFG_CHEN | DMA_CFG_DAINC | DMA_CFG_SAINC | DMA_CFG_BURSIZE |
                          DMA_CFG_TPTYPE | DMA_CFG_REQSRC);
    tmpreg |= (uint32_t)(DMA_InitStruct->DMA_Priority | DMA_InitStruct->DMA_CircularMode |
 ...
 *              - DMA_State_BUSY:DMA reads the source address data and writes to the
 * destination address
 ...
 *                  - DMA2 :select DMA2 peripherals
 *                  - DMA3 :select DMA3 peripherals
 * @param DMA_DMARequest[in]: specifies the DMA interrupt sources to be enabled or
 * disabled.
