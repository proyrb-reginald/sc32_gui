...
/**
 * @brief  Enables or disables the WDT peripheral.The IFB(AHB_CFG->IFBEN) must be enabled
 * first before the WDT enable can be configured.
...
void OPTION_IAPPORA(uint16_t IAPPROAST, uint16_t IAPPROAED) {
    OPT->OPINX = 0xC3;
    OPT->OPREG = (IAPPROAST >> 8);
    OPT->OPINX = 0xC4;
    OPT->OPREG = (uint8_t)IAPPROAST;
    OPT->OPINX = 0xC5;
    OPT->OPREG = (IAPPROAED >> 8);
    OPT->OPINX = 0xC6;
    OPT->OPREG = (uint8_t)IAPPROAED;
}
...
void OPTION_IAPPORB(uint16_t IAPPROBST, uint16_t IAPPROBED) {
    OPT->OPINX = 0xC7;
    OPT->OPREG = (IAPPROBST >> 8);
    OPT->OPINX = 0xC8;
    OPT->OPREG = (uint8_t)IAPPROBST;
    OPT->OPINX = 0xC9;
    OPT->OPREG = (IAPPROBED >> 8);
    OPT->OPINX = 0xCA;
    OPT->OPREG = (uint8_t)(IAPPROBED);
}
