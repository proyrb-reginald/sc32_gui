...
/**
 * @brief  Initializes the peripheral CRC register according to the parameters specified
 * in CRC_InitStruct
...
uint32_t CRC_Accumulate(CRC_InputData_Format_TypeDef InputDataFormat,
    ...
    switch (InputDataFormat) {
        ...
        case CRC_InputData_Format_HALFWORDS:
            temp = CRC_Handle_16((uint16_t *)(void *)pBuffer,
...
static uint32_t CRC_Handle_8(uint8_t pBuffer[], uint32_t BufferLength) {
    ...
    /* Processing time optimization: 4 bytes are entered in a row with a single word
     * write, last bytes must be carefully fed to the CRC calculator to ensure a correct
     * type handling by the peripheral */
    ...
    if ((BufferLength % 4U) != 0U) {
        if ((BufferLength % 4U) == 1U) {
            *(__IO uint8_t *)(__IO void *)(&CRC->CRC_DR) =
                pBuffer[4U * i]; /* Derogation MisraC2012 R.11.5 */
        }
        if ((BufferLength % 4U) == 2U) {
            data = (uint16_t)(((uint16_t)(pBuffer[4U * i]) << 8U) |
                              (uint16_t)pBuffer[(4U * i) + 1U]);
            pReg = (__IO uint16_t *)(__IO void *)(&CRC->CRC_DR); /* Derogation MisraC2012
                                                                    R.11.5 */
            *pReg = data;
        }
...
static uint32_t CRC_Handle_16(uint16_t pBuffer[], uint32_t BufferLength) {
    ...
    /* Processing time optimization: 2 HalfWords are entered in a row with a single word
     * write, in case of odd length, last HalfWord must be carefully fed to the CRC
     * calculator to ensure a correct type handling by the peripheral */
    for (i = 0U; i < (BufferLength / 2U); i++) {
        CRC->CRC_DR =
            ((uint32_t)pBuffer[2U * i] << 16U) | (uint32_t)pBuffer[(2U * i) + 1U];
    }
