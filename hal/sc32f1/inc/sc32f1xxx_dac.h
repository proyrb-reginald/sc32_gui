...
#ifndef __sc32f1xxx_DAC_H
#define __sc32f1xxx_DAC_H
...
#define IS_DAC_PRESCALER(PRESCALER)                                                      \
    (((PRESCALER) == DAC_Channel_0) || ((PRESCALER) == DAC_Channel_1))
/**
 * @}
 */
...
typedef enum {
    DAC_RefSource_VDD =
        (uint32_t)(0x00 << DAC_CFG_REFSEL_Pos), /*!< DAC reference voltage is VDD */
    DAC_RefSource_VREF =
        (uint32_t)(0x01 << DAC_CFG_REFSEL_Pos), /*!< DAC reference voltage is Vref */
} DAC_REF_TypeDef;

#define IS_DAC_VREF(VREF)                                                                \
    (((VREF) == DAC_RefSource_VDD) || ((VREF) == DAC_RefSource_VREF))
/**
 * @}
 */
 ...
 */
typedef enum {
    DAC_DACSTA_Endconversion =
        (uint8_t)(0x00 << DAC_STS_OVFIF_Pos), /*!< DAC end conversion  */
    DAC_DACSTA_Inconversion =
        (uint8_t)(0x01 << DAC_STS_OVFIF_Pos), /*!<DAC is being converted */
} DAC_DACSTA_TypeDef;

#define IS_DAC_DACSTA(DACSTA)                                                            \
    ((DACSTA) == DAC_DACSTA_Endconversion || (DACSTA) == DAC_DACSTA_Inconversion)

#define IS_GET_DAC_FLAG(FLAG)                                                            \
    ((FLAG) == DAC_DACSTA_Endconversion || (FLAG) == DAC_DACSTA_Inconversion)
/**
 * @}
 */
...
void DAC_ChannelConfig(DAC_TypeDef *       DACx,
                       DAC_Channel_TypeDef DAC_Channel,
