...
#define IS_UART_PRESCALER(PRESCALER)                                                     \
    (((PRESCALER) == UART_PRESCALER_12) || ((PRESCALER) == UART_PRESCALER_4))
/**
 * @}
 */
...
typedef enum {
    UART_Mode_8B =
        (uint32_t)(0X00U << UART_CON_SM01_Pos), /*!< UART Mode:8-bit half duplex    */
    UART_Mode_10B =
        (uint32_t)(0X01U << UART_CON_SM01_Pos), /*!< UART Mode:10-bit full duplex    */
    UART_Mode_11B =
        (uint32_t)(0X03U << UART_CON_SM01_Pos), /*!< UART Mode:11-bit full duplex    */
} UART_Mode_Typedef;

#define IS_UART_Mode(MODE)                                                               \
    (((MODE) == UART_Mode_8B) || ((MODE) == UART_Mode_10B) || ((MODE) == UART_Mode_11B))
/**
 * @}
 */
 ...
 */
typedef enum {
    UART_PinRemap_Default = (uint32_t)0x00
                            << UART_CON_SPOS_Pos, /*!< TIM Pin Remap: Disable */
    UART_PinRemap_A =
        (uint32_t)(0x01 << UART_CON_SPOS_Pos), /*!< TIM Pin Remap: Remap mode A */
} UART_PinRemap_TypeDef;

#define IS_UART_PINREMAP(PINREMAP)                                                       \
    (((PINREMAP) == UART_PinRemap_Default) || ((PINREMAP) == UART_PinRemap_Enable))
/**
 * @}
 */
...
#define IS_GET_UART_FLAG(FLAG)                                                           \
    (((FLAG) == UART_Flag_RX) || ((FLAG) == UART_Flag_TX) || ((FLAG) == UART_Flag_BK) || \
     ((FLAG) == UART_Flag_SLVYN) || ((FLAG) == UART_Flag_SLVHE) ||                       \
     ((FLAG) == UART_Flag_SY))
/**
 * @}
 */
...
#define IS_UART_DMAREQ(DMAREQ)                                                           \
    ((((DMAREQ) & (uint8_t)0x3F) == 0x00) && ((DMAREQ) != 0x00))
/**
 * @}
 */
...
typedef enum {
    UART_MASTER =
        (uint32_t)(0x00 << UART_CON_SLVEN_Pos |
                   (0x01 << UART_CON_FUNCSEL_Pos)), /*!< UART_LINMODE: UART_MASTER */
    UART_SLAVER =
        (uint32_t)(0x01 << UART_CON_SLVEN_Pos |
                   (0x01 << UART_CON_FUNCSEL_Pos)), /*!< UART_LINMODE: UART_SLAVER*/
} UART_LINMODE_TypeDef;

#    define IS_UART_LINMODE(LINMODE)                                                     \
        (((LINMODE) == UART_MASTER) || ((LINMODE) == UART_SLAVER))
/**
 * @}
 */
 ...
 */
typedef enum {
    UART_BKSIZE_10 = (uint32_t)0x00
                     << UART_CON_BKSIZE_Pos, /*!< UART_BKSIZE: UART_BKSIZE_10 */
    UART_BKSIZE_13 =
        (uint32_t)(0x01 << UART_CON_BKSIZE_Pos), /*!< UART_BKSIZE:UART_BKSIZE_13 */
} UART_BKSIZE_TypeDef;

#    define IS_UART_BKSIZE(BKSIZE)                                                       \
        (((BKSIZE) == UART_BKSIZE_10) || ((BKSIZE) == UART_BKSIZE_13))
...
#    define IS_UART_LBDL(LBDL) (((LBDL) == UART_LBDL_10) || ((LBDL) == UART_LBDL_11))
#endif
/* End of enumerations -----------------------------------------------------*/
...
#endif
#if defined(SC32R803)
#    define IS_UART_REMAP_PERIPH(PERIPH)                                                 \
        (((PERIPH) == UART0) || ((PERIPH) == UART1) || ((PERIPH) == UART2) ||            \
         ((PERIPH) == UART3) || ((PERIPH) == UART4) || ((PERIPH) == UART5))
#endif
#if defined(SC32f15xx) || defined(SC32R601)
...
typedef struct {
    uint32_t
        UART_ClockFrequency; /*!<  Specifies the clock division.
                                                                           This parameter
                                can be a value of @ref UART_ClockDivision */

    uint32_t UART_BaudRate; /*!< This member configures the UART communication baud rate.
                                            When UART_Mode is mode 0 This parameter can be
                               a value of @ref UART_Prescaler_TypeDef. */

    uint32_t
        UART_Mode; /*!< Specifies the counter dierction.
                                                                                   This
                      parameter can be a value of @ref UART_Mode_Typedef */
} UART_InitTypeDef;
/**
 * @}
 */
...
extern UART_TypeDef * printf_uart;
