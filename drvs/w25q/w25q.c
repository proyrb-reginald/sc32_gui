#include "w25q_cfg.h"
#include "w25q_if.h"
#include INC_LOG
#include INC_RTOS
#include <string.h>

static rt_sem_t dma_done_sem;

__attribute__((noinline)) static void gpio_init(void) {
    GPIO_InitTypeDef gpio_struct = {.GPIO_DriveLevel = 0, .GPIO_Mode = GPIO_Mode_OUT_PP};
    gpio_struct.GPIO_Pin         = CHIP_GPIO_PIN;
    GPIO_Init(CHIP_GPIO_GRP, &gpio_struct);
}

__attribute__((optnone)) void w25q_ctl(const w25q_cmd_t * const cmd) {
    GPIO_WriteBit(CHIP_GPIO_GRP, CHIP_GPIO_PIN, 0);

    /********** 发送指令 **********/

    if (cmd->ins == Reset) {
        QSPI_Write_ComSet(QSPI_DEV, QSPI_LMode_1Line, QSPI_DWidth_16bit,
                          QSPI_CLKONLY_OFF);
        QSPI_SendData16(QSPI_DEV, Reset);
    } else {
        QSPI_Write_ComSet(QSPI_DEV, QSPI_LMode_1Line, QSPI_DWidth_8bit, QSPI_CLKONLY_OFF);
        QSPI_SendData8(QSPI_DEV, cmd->ins);
    }
    while (QSPI_GetFlagStatus(QSPI_DEV, QSPI_Flag_BUSY)) {}

    /********** 发送参数 **********/

    switch (cmd->ins) {
        case ReadUniqueID:
            /********** 发送4个空字节 **********/

            QSPI_Write_ComSet(QSPI_DEV, QSPI_LMode_1Line, QSPI_DWidth_8bit,
                              QSPI_CLKONLY_OFF);
            for (uint8_t i = 0; i < 4; i++) {
                QSPI_SendData8(QSPI_DEV, 0);
                while (QSPI_GetFlagStatus(QSPI_DEV, QSPI_Flag_BUSY)) {}
            }
            break;
        case ReadDataSPI:
            /********** 从高到低发送3个字节的地址 **********/
            break;
        case ReadDataQSPI:
            /********** 从高到低发送3个字节的地址 **********/

            QSPI_Write_ComSet(QSPI_DEV, QSPI_LMode_4Line, QSPI_DWidth_8bit,
                              QSPI_CLKONLY_OFF);
            QSPI_SendMultipleData(QSPI_DEV, (void *)cmd->adr.byte, 3);

            /********** 发送1个固定字节 **********/

            QSPI_SendData8(QSPI_DEV, 0xff);
            while (QSPI_GetFlagStatus(QSPI_DEV, QSPI_Flag_BUSY)) {}

            /********** 发送2个空字节 **********/

            QSPI_Write_ComSet(QSPI_DEV, QSPI_LMode_4Line, QSPI_DWidth_16bit,
                              QSPI_CLKONLY_OFF);
            QSPI_SendData16(QSPI_DEV, 0);
            while (QSPI_GetFlagStatus(QSPI_DEV, QSPI_Flag_BUSY)) {}
            break;
        case Erase4KB:
        case Erase32KB:
        case Erase64KB:
            /********** 从高到低发送3个字节的地址 **********/

            QSPI_Write_ComSet(QSPI_DEV, QSPI_LMode_1Line, QSPI_DWidth_8bit,
                              QSPI_CLKONLY_OFF);
            QSPI_SendMultipleData(QSPI_DEV, (void *)cmd->adr.byte, 3);
            break;
        case ProgramSPI:
            /********** 从高到低发送3个字节的地址 **********/
            break;
        case ProgramQSPI:
            /********** 从高到低发送3个字节的地址 **********/

            QSPI_Write_ComSet(QSPI_DEV, QSPI_LMode_1Line, QSPI_DWidth_8bit,
                              QSPI_CLKONLY_OFF);
            QSPI_SendMultipleData(QSPI_DEV, (void *)cmd->adr.byte, 3);
        default: break;
    }

    /********** 传输数据 **********/

    if ((cmd->data != NULL) && (cmd->size > 0)) {
        switch (cmd->ins) {
            case ReadUniqueID:
                /********** 接收8个字节 **********/

                QSPI_Read_ComSet(QSPI_DEV, QSPI_LMode_1Line, QSPI_DWidth_8bit, cmd->size);
                QSPI_ReceiveMultipleData(QSPI_DEV, (uint32_t *)cmd->data, cmd->size);
                break;
            case ReadSR1:
            case ReadSR2:
            case ReadSR3:
                /********** 接收1个字节时钟 **********/

                QSPI_Read_ComSet(QSPI_DEV, QSPI_LMode_1Line, QSPI_DWidth_8bit, 1);
                QSPI_CLKONLYSet(QSPI_DEV, QSPI_CLKONLY_ON);
                while (QSPI_GetFlagStatus(QSPI_DEV, QSPI_Flag_BUSY)) {}
                cmd->data[0] = QSPI_ReceiveData8(QSPI_DEV);
                break;
            case ReadDataSPI:
                /********** 连续接收 **********/
                break;
            case ReadDataQSPI:
                /********** 连续接收 **********/

#ifdef USE_DMA
                QSPI_Read_ComSet(QSPI_DEV, QSPI_LMode_4Line, QSPI_DWidth_8bit, cmd->size);
                DMA_SetDstAddress(DMA_RX_DEV, (uint32_t)cmd->data);
                DMA_SetCurrDataCounter(DMA_RX_DEV, cmd->size);
                DMA_Cmd(DMA_RX_DEV, ENABLE);
                QSPI_DMACmd(QSPI_DEV, QSPI_DMAReq_RX, ENABLE);
                rt_sem_take(dma_done_sem, RT_WAITING_NO);
                QSPI_CLKONLYSet(QSPI_DEV, QSPI_CLKONLY_ON);
                rt_sem_take(dma_done_sem, RT_WAITING_FOREVER);
#else
                QSPI_Read_ComSet(QSPI_DEV, QSPI_LMode_4Line, QSPI_DWidth_8bit, cmd->size);
                QSPI_ReceiveMultipleData(QSPI_DEV, (void *)cmd->data, cmd->size);
#endif
                break;
            case ProgramSPI:
                /********** 连续发送 **********/
                break;
            case ProgramQSPI:
                /********** 连续发送 **********/

#ifdef USE_DMA
                QSPI_Write_ComSet(QSPI_DEV, QSPI_LMode_4Line, QSPI_DWidth_8bit,
                                  QSPI_CLKONLY_OFF);
                DMA_SetSrcAddress(DMA_TX_DEV, (uint32_t)cmd->data);
                DMA_SetCurrDataCounter(DMA_TX_DEV, cmd->size);
                DMA_Cmd(DMA_TX_DEV, ENABLE);
                QSPI_DMACmd(QSPI_DEV, QSPI_DMAReq_TX, ENABLE);
                rt_sem_take(dma_done_sem, RT_WAITING_NO);
                DMA_SoftwareTrigger(DMA_TX_DEV);
                rt_sem_take(dma_done_sem, RT_WAITING_FOREVER);
#else
                QSPI_Write_ComSet(QSPI_DEV, QSPI_LMode_4Line, QSPI_DWidth_8bit,
                                  QSPI_CLKONLY_OFF);
                QSPI_SendMultipleData(QSPI_DEV, (void *)cmd->data, cmd->size);
#endif
                break;
            default: break;
        }
    }

    GPIO_WriteBit(CHIP_GPIO_GRP, CHIP_GPIO_PIN, 1);
}

uint8_t w25q_init(void) {
    if (dma_done_sem == RT_NULL) {
        dma_done_sem = rt_sem_create("dma_done", 0, RT_IPC_FLAG_FIFO);
        if (dma_done_sem == RT_NULL) {
            PRTF_OS_LOG(ERRO_LOG, "dma_done_sem create fail!\n");
            return RT_ERROR;
        }
    }

    gpio_init();

    GPIO_WriteBit(CHIP_GPIO_GRP, CHIP_GPIO_PIN, 1);

    uint8_t    data[8] = {0};
    w25q_cmd_t cmd     = {.data = data};

    cmd.ins  = Reset;
    cmd.size = 0;
    w25q_ctl(&cmd);
    RTOS_DELAY_IF(1);

    cmd.ins  = ReadUniqueID;
    cmd.size = sizeof(data);
    w25q_ctl(&cmd);

    const uint64_t id = *(uint64_t *)data;
    PRTF_OS_LOG(INFO_LOG, "id: 0x");
    for (uint8_t i = 0; i < sizeof(data); i++) {
        PRTF_LOG(INFO_LOG, "%x", data[i]);
    }
    PRTF_LOG(INFO_LOG, "\n");

    cmd.ins  = WriteDisable;
    cmd.size = 0;
    w25q_ctl(&cmd);
    PRTF_OS_LOG(INFO_LOG, "Write Disable!\n");

    cmd.ins  = ReadSR1;
    cmd.size = 1;
    w25q_ctl(&cmd);
    PRTF_OS_LOG(INFO_LOG, "SR1: 0x%X!\n", data[0]);

    cmd.ins = ReadSR2;
    w25q_ctl(&cmd);
    PRTF_OS_LOG(INFO_LOG, "SR2: 0x%X!\n", data[0]);

    cmd.ins = ReadSR3;
    w25q_ctl(&cmd);
    PRTF_OS_LOG(INFO_LOG, "SR3: 0x%X!\n", data[0]);

    PRTF_OS_LOG(NEWS_LOG, "init ok!\n");

    return 0;
}

#ifdef USE_TEST

#    define DATA_SIZE 256
#    define SCT_CNT 4

static uint8_t test_r_data[DATA_SIZE] = {0};
static uint8_t test_w_data[DATA_SIZE] = {0};

__attribute__((noreturn)) void w25q_test(void * thread_args) {
    PRTF_OS_LOG(NEWS_LOG, "start test!\n");

    for (uint16_t i = 0; i < DATA_SIZE; i++) {
        test_w_data[i] = i;
    }

    w25q_init();

    w25q_cmd_t cmd;
    while (1) {
        for (uint8_t i = 0; i < SCT_CNT; i++) {
            PRTF_OS_LOG(INFO_LOG, "[[sct:%u]]\n", i);

            /********** 擦除扇区 **********/

            cmd.ins  = WriteEnable;
            cmd.size = 0;
            w25q_ctl(&cmd);

            cmd.ins = Erase4KB;
            {
                uint32_t tmp    = i * SCT_CNT;
                cmd.adr.byte[2] = tmp >> (0 * 8);
                cmd.adr.byte[1] = tmp >> (1 * 8);
                cmd.adr.byte[0] = tmp >> (2 * 8);
            }
            w25q_ctl(&cmd);

            cmd.ins  = ReadSR1;
            cmd.data = test_r_data;
            cmd.size = 1;
            do {
                RTOS_DELAY_IF(3);
                w25q_ctl(&cmd);
                PRTF_OS_LOG(INFO_LOG, "SR1: 0x%x\n", test_r_data[0]);
            } while (test_r_data[0] & 0x1);
            PRTF_OS_LOG(INFO_LOG, "erase sct: %u adr: 0x%02x%02x%02x\n", i,
                        cmd.adr.byte[0], cmd.adr.byte[1], cmd.adr.byte[2]);

            /********** 从扇区第一页读数据 **********/

            cmd.ins  = ReadDataQSPI;
            cmd.data = test_r_data;
            cmd.size = DATA_SIZE;
            w25q_ctl(&cmd);
            PRTF_OS_LOG(INFO_LOG, "sct: %u adr: 0x%x size: %u data:\n", i,
                        (4 * 1024) * SCT_CNT, DATA_SIZE);
            for (uint16_t i = 0; i < DATA_SIZE; i++) {
                PRTF_LOG(INFO_LOG, "0x%02x ", test_r_data[i]);
            }
            PRTF_LOG(INFO_LOG, "\n");

            /********** 向扇区第一页写数据 **********/

            cmd.ins  = WriteEnable;
            cmd.size = 0;
            w25q_ctl(&cmd);

            cmd.ins = ProgramQSPI;
            {
                uint32_t tmp    = i * SCT_CNT;
                cmd.adr.byte[2] = tmp >> (0 * 8);
                cmd.adr.byte[1] = tmp >> (1 * 8);
                cmd.adr.byte[0] = tmp >> (2 * 8);
            }
            cmd.data = test_w_data;
            cmd.size = DATA_SIZE;
            w25q_ctl(&cmd);

            cmd.ins  = ReadSR1;
            cmd.data = test_r_data;
            cmd.size = 1;
            do {
                RTOS_DELAY_IF(2);
                w25q_ctl(&cmd);
                PRTF_OS_LOG(INFO_LOG, "SR1: 0x%x\n", test_r_data[0]);
            } while (test_r_data[0] & 0x1);
            PRTF_OS_LOG(INFO_LOG, "write sct: %u adr: 0x%02x%02x%02x size: %u\n", i,
                        cmd.adr.byte[0], cmd.adr.byte[1], cmd.adr.byte[2], DATA_SIZE);

            /********** 从扇区第一页读数据 **********/

            cmd.ins  = ReadDataQSPI;
            cmd.data = test_r_data;
            cmd.size = DATA_SIZE;
            w25q_ctl(&cmd);
            PRTF_OS_LOG(INFO_LOG, "sct: %u adr: 0x%x size: %u data:\n", i,
                        (4 * 1024) * SCT_CNT, DATA_SIZE);
            for (uint16_t i = 0; i < DATA_SIZE; i++) {
                PRTF_LOG(INFO_LOG, "0x%02x ", test_r_data[i]);
            }
            PRTF_LOG(INFO_LOG, "\n");
        }
    }
}

#endif

#ifdef USE_DMA

__attribute__((always_inline)) void w25q_dma_tx_irq(void) {
    QSPI_DMACmd(QSPI_DEV, QSPI_DMAReq_TX, DISABLE);
    DMA_Cmd(DMA_TX_DEV, DISABLE);
    rt_sem_release(dma_done_sem);
}

__attribute__((always_inline)) void w25q_dma_rx_irq(void) {
    QSPI_DMACmd(QSPI_DEV, QSPI_DMAReq_RX, DISABLE);
    DMA_Cmd(DMA_RX_DEV, DISABLE);
    rt_sem_release(dma_done_sem);
}

#endif