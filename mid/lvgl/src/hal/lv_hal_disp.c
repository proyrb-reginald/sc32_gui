...
static void set_px_true_color_alpha(lv_disp_drv_t * disp_drv,
                                    uint8_t *       buf,
                                    lv_coord_t      buf_w,
                                    lv_coord_t      x,
                                    lv_coord_t      y,
...
static void set_px_cb_alpha2(lv_disp_drv_t * disp_drv,
                             uint8_t *       buf,
                             lv_coord_t      buf_w,
                             lv_coord_t      x,
                             lv_coord_t      y,
...
static void set_px_cb_alpha4(lv_disp_drv_t * disp_drv,
                             uint8_t *       buf,
                             lv_coord_t      buf_w,
                             lv_coord_t      x,
                             lv_coord_t      y,
...
static void set_px_alpha_generic(lv_img_dsc_t * d,
                                 lv_coord_t     x,
                                 lv_coord_t     y,
                                 lv_color_t     color,
                                 lv_opa_t       opa);
 ...
 * @param buf2 Optionally specify a second buffer to make image rendering and image
 * flushing (sending to the display) parallel. In the `disp_drv->flush` you should use DMA
 * or similar hardware to send the image to the display in the background. It lets LVGL to
 * render next frame into the other buffer while previous is being sent. Set to `NULL` if
 * unused.
 ...
 * @param driver pointer to an initialized 'lv_disp_drv_t' variable. Only its pointer is
 * saved!
...
lv_disp_t * lv_disp_drv_register(lv_disp_drv_t * driver) {
    ...
    disp_def = disp; /*Temporarily change the default screen to create the default screens
                        on the new display*/
    ...
    lv_timer_ready(disp->refr_timer); /*Be sure the screen will be refreshed immediately
                                         on start up*/
...
lv_coord_t lv_disp_get_hor_res(lv_disp_t * disp) {
    ...
    if (disp == NULL) {
        ...
        switch (disp->driver->rotated) {
            case LV_DISP_ROT_90:
            case LV_DISP_ROT_270: return disp->driver->ver_res;
            default: return disp->driver->hor_res;
        }
    }
}
...
lv_coord_t lv_disp_get_ver_res(lv_disp_t * disp) {
    ...
    if (disp == NULL) {
        ...
        switch (disp->driver->rotated) {
            case LV_DISP_ROT_90:
            case LV_DISP_ROT_270: return disp->driver->hor_res;
            default: return disp->driver->ver_res;
        }
    }
}
...
lv_coord_t lv_disp_get_offset_x(lv_disp_t * disp) {
    ...
    if (disp == NULL) {
        ...
        switch (disp->driver->rotated) {
            case LV_DISP_ROT_90: return disp->driver->offset_y;
            ...
            default: return disp->driver->offset_x;
        }
    }
}
...
lv_coord_t lv_disp_get_offset_y(lv_disp_t * disp) {
    ...
    if (disp == NULL) {
        ...
        switch (disp->driver->rotated) {
            case LV_DISP_ROT_90: return disp->driver->offset_x;
            case LV_DISP_ROT_180:
            ...
            default: return disp->driver->offset_y;
        }
    }
}
 ...
 * Can be called from `flush_cb` to execute some special display refreshing if needed when
 * all areas area flushed.
 * @param disp_drv pointer to display driver
 * @return true: it's the last area to flush; false: there are other areas too which will
 * be refreshed soon
 */
 ...
 * @return the next display or NULL if no more. Give the first display when the parameter
 * is NULL
 */
...
static void set_px_true_color_alpha(lv_disp_drv_t * disp_drv,
    ...
    uint8_t * buf_px =
        buf + (buf_w * y * LV_IMG_PX_SIZE_ALPHA_BYTE + x * LV_IMG_PX_SIZE_ALPHA_BYTE);
