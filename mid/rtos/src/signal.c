 ...
 * 2018/09/17     Jesven       fix: in _signal_deliver RT_THREAD_STAT_MASK to
 * RT_THREAD_STAT_SIGNAL_MASK 2018/11/22     Jesven       in smp version
 * rt_hw_context_switch_to add a param
 */
...
/**
 * @brief    This function will wait for the arrival of the set signal. If it does not
 * wait for this signal, the thread will be suspended until it waits for this signal or
 * the waiting time exceeds the specified timeout: timeout.
 *
 * @param    set is the set of signal values to be waited for. Use the function
 *           sigaddset() to add the signal.
 *
 * @param    si is a pointer to the received signal info. If you don't care about this
 ...
 * is successful. If the return value is any other values, it means that the signal wait
 * failed.
 */
int rt_signal_wait(const rt_sigset_t * set, rt_siginfo_t * si, rt_int32_t timeout) {
    ...
    if (timeout != RT_WAITING_FOREVER) {
        ...
        rt_timer_control(&(tid->thread_timer), RT_TIMER_CTRL_SET_TIME, &timeout);
...
__done:
    ...
    while (si_node) {
        ...
        if (si_node->list.next) {
            si_node =
                (void *)rt_slist_entry(si_node->list.next, struct siginfo_node, list);
        } else {
            si_node = RT_NULL;
        }
    }
 ...
 *
 * @return   Return the operation status. When the return value is RT_EOK, the operation
 * is successful. If the return value is any other values, it means that the signal send
 * failed.
 */
