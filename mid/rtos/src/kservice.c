 ...
 * 2013-09-24     aozima       make sure the device is in STREAM mode when used by
 * rt_kprintf. 2015-07-06     Bernard      Add rt_assert_handler routine. 2021-02-28 Meco
 * Man     add RT_KSERVICE_USING_STDLIB 2021-12-20     Meco Man     implement rt_strcpy()
...
RT_WEAK void rt_hw_us_delay(rt_uint32_t us) {
    (void)us;
    RT_DEBUG_LOG(RT_DEBUG_DEVICE,
                 ("rt_hw_us_delay() doesn't support for this board."
                  "Please consider implementing rt_hw_us_delay() in another file.\n"));
}

static const char * rt_errno_strs[] = {"OK",      "ERROR",   "ETIMOUT", "ERSFULL",
                                       "ERSEPTY", "ENOMEM",  "ENOSYS",  "EBUSY",
                                       "EIO",     "EINTRPT", "EINVAL",  "EUNKNOW"};
...
const char * rt_strerror(rt_err_t error) {
    ...
    return (error > RT_EINVAL + 1) ? rt_errno_strs[RT_EINVAL + 1] : rt_errno_strs[error];
}
...
rt_int32_t rt_memcmp(const void * cs, const void * ct, rt_size_t count) {
    ...
    for (su1 = (const unsigned char *)cs, su2 = (const unsigned char *)ct; 0 < count;
         ++su1, ++su2, count--)
        if ((res = *su1 - *su2) != 0)
            break;
...
rt_inline void   _smem_info(rt_size_t * total, rt_size_t * used, rt_size_t * max_used) {
    if (total)
...
#        define _MEM_REALLOC(_ptr, _newsize) rt_smem_realloc(system_heap, _ptr, _newsize)
...
rt_inline void   _slab_info(rt_size_t * total, rt_size_t * used, rt_size_t * max_used) {
    if (total)
        *total = system_heap->total;
    if (used)
        *used = system_heap->used;
...
#        define _MEM_REALLOC(_ptr, _newsize) rt_slab_realloc(system_heap, _ptr, _newsize)
 ...
 */
void rt_assert_set_hook(void (*hook)(const char * ex,
                                     const char * func,
                                     rt_size_t    line)) {
    rt_assert_hook = hook;
}
