...
void rt_thread_idle_init(void) {
    ...
    for (i = 0; i < _CPUS_NR; i++) {
        ...
        rt_thread_init(&idle[i], tidle_name, rt_thread_idle_entry, RT_NULL,
                       &rt_thread_stack[i][0], sizeof(rt_thread_stack[i]),
    ...
    rt_thread_init(&rt_system_thread, "tsystem", rt_thread_system_entry, RT_NULL,
                   rt_system_stack, sizeof(rt_system_stack), RT_THREAD_PRIORITY_MAX - 2,
                   32);
    /* startup */
