 ...
 * 2021-02-24     Meco Man     rearrange rt_thread_control() - schedule the thread when
 * close it 2021-11-15     THEWON       Remove duplicate work between idle and
 * _thread_exit 2021-12-27     Meco Man     remove .init_priority 2022-01-07     Gabriel
 * Moving __on_rt_xxxxx_hook to thread.c 2022-01-24     THEWON       let rt_thread_sleep
 * return thread->error when using signal
 */
...
static rt_err_t _thread_init(struct rt_thread * thread,
    ...
    rt_timer_init(&(thread->thread_timer), thread->name, _thread_timeout, thread, 0,
 ...
 *
 * @return  Return the operation status. If the return value is RT_EOK, the function is
 * successfully executed. If the return value is any other values, it means this operation
 * failed.
 */
rt_err_t rt_thread_init(struct rt_thread * thread,
    ...
    return _thread_init(thread, name, entry, parameter, stack_start, stack_size, priority,
                        tick);
}
 ...
 * @param   tick is the time slice if there are same priority thread.
 *
 * @return  If the return value is a rt_thread structure pointer, the function is
 * successfully executed. If the return value is RT_NULL, it means this operation failed.
 */
...
/**
 * @brief   This function will let current thread sleep for some ticks. Change current
 * thread state to suspend, when the thread timer reaches the tick value, scheduler will
 * awaken this thread.
 *
...
/**
 * @brief   This function will suspend the specified thread and change it to suspend
 * state.
 *
 * @note    This function ONLY can suspend current thread itself.
 *              rt_thread_suspend(rt_thread_self());
 *
 *          Do not use the rt_thread_suspend to suspend other threads. You have no way of
 * knowing what code a thread is executing when you suspend it. If you suspend a thread
 * while sharing a resouce with other threads and occupying this resouce, starvation can
 * occur very easily.
 *
