...
#    define MEMITEM_SIZE(item)                                                           \
        ((rt_ubase_t)item->next - (rt_ubase_t)item - RT_MEMHEAP_SIZE)
...
#    endif /* RT_USING_MEMTRACE */
    ...
    item->next = (struct rt_memheap_item *)((rt_uint8_t *)item + memheap->available_size +
                                            RT_MEMHEAP_SIZE);
    item->prev = item->next;
...
void * rt_memheap_alloc(struct rt_memheap * heap, rt_size_t size) {
    ...
    RT_DEBUG_LOG(RT_DEBUG_MEMHEAP,
                 ("allocate %d on heap:%8.*s", size, RT_NAME_MAX, heap->parent.name));
    ...
    if (size < heap->available_size) {
        ...
        if (free_size >= size) {
            ...
            if (free_size >= (size + RT_MEMHEAP_SIZE + RT_MEMHEAP_MINIALLOC)) {
                ...
                /* split the block. */
                new_ptr = (struct rt_memheap_item *)(((rt_uint8_t *)header_ptr) + size +
                                                     RT_MEMHEAP_SIZE);

                RT_DEBUG_LOG(
                    RT_DEBUG_MEMHEAP,
                    ("split: block[0x%08x] nextm[0x%08x] prevm[0x%08x] to new[0x%08x]\n",
                     header_ptr, header_ptr->next, header_ptr->prev, new_ptr));
...
#    ifdef RT_USING_MEMTRACE
                rt_memset(new_ptr->owner_thread_name, ' ',
...
#    ifdef RT_USING_MEMTRACE
            if (rt_thread_self())
                rt_memcpy(header_ptr->owner_thread_name, rt_thread_self()->name,
                          sizeof(header_ptr->owner_thread_name));
            else
                rt_memcpy(header_ptr->owner_thread_name, "NONE",
                          sizeof(header_ptr->owner_thread_name));
#    endif /* RT_USING_MEMTRACE */
            ...
            if (nextsize + oldsize > newsize + RT_MEMHEAP_MINIALLOC) {
                ...
                RT_DEBUG_LOG(
                    ...
                    ("remove block: block[0x%08x], next_free 0x%08x, prev_free 0x%08x",
                     next_ptr, next_ptr->next_free, next_ptr->prev_free));
                ...
                RT_DEBUG_LOG(RT_DEBUG_MEMHEAP,
                             ("new free block: block[0x%08x] nextm[0x%08x] prevm[0x%08x]",
                              next_ptr, next_ptr->next, next_ptr->prev));
...
#    endif /* RT_USING_MEMTRACE */
                ...
                RT_DEBUG_LOG(RT_DEBUG_MEMHEAP,
                             ("new ptr: next_free 0x%08x, prev_free 0x%08x",
                              next_ptr->next_free, next_ptr->prev_free));
                if (heap->locked == RT_FALSE) {
                    /* release lock */
                    rt_sem_release(&(heap->lock));
                }
    ...
    /* split the block. */
    new_ptr = (struct rt_memheap_item *)(((rt_uint8_t *)header_ptr) + newsize +
                                         RT_MEMHEAP_SIZE);
...
int memheapcheck(int argc, char * argv[]) {
    ...
    for (node = list->next; node != list; node = node->next) {
        ...
        for (item = heap->block_list; item->next != heap->block_list; item = item->next) {
            /* check magic */
            if (!((item->magic & (RT_MEMHEAP_MAGIC | RT_MEMHEAP_FREED)) ==
                      (RT_MEMHEAP_MAGIC | RT_MEMHEAP_FREED) ||
                  (item->magic & (RT_MEMHEAP_MAGIC | RT_MEMHEAP_USED)) ==
                      (RT_MEMHEAP_MAGIC | RT_MEMHEAP_USED))) {
                has_bad = RT_TRUE;
                break;
            }
            /* check pool_ptr */
            ...
            /* check next and prev */
            if (!((rt_ubase_t)item->next <=
                      (rt_ubase_t)((rt_ubase_t)heap->start_addr + heap->pool_size) &&
                  (rt_ubase_t)item->prev >= (rt_ubase_t)heap->start_addr) &&
                (rt_ubase_t)item->next ==
                    RT_ALIGN((rt_ubase_t)item->next, RT_ALIGN_SIZE) &&
                (rt_ubase_t)item->prev ==
                    RT_ALIGN((rt_ubase_t)item->prev, RT_ALIGN_SIZE)) {
                has_bad = RT_TRUE;
                break;
            }
            /* check item */
...
int memheaptrace(int argc, char * argv[]) {
    ...
    for (node = list->next; node != list; node = node->next) {
        ...
        /* memheap item */
        for (header_ptr = mh->block_list; header_ptr->next != mh->block_list;
            ...
            rt_kprintf("] %c%c%c%c\n", header_ptr->owner_thread_name[0],
                       header_ptr->owner_thread_name[1], header_ptr->owner_thread_name[2],
