 ...
 *                             add _scheduler_get_highest_priority_thread to find highest
 * priority task rt_schedule_insert_thread won't insert current task to ready queue in smp
 * version, rt_hw_context_switch_interrupt maybe switch to new task directly 2022-01-07
 * Gabriel      Moving __on_rt_xxxxx_hook to scheduler.c
 */
...
#    define __on_rt_scheduler_switch_hook(tid)                                           \
        __ON_HOOK_ARGS(rt_scheduler_switch_hook, (tid))
#endif
...
#    ifdef ARCH_CPU_STACK_GROWS_UPWARD
    else if ((rt_ubase_t)thread->sp >
             ((rt_ubase_t)thread->stack_addr + thread->stack_size)) {
...
_scheduler_get_highest_priority_thread(rt_ubase_t * highest_prio) {
...
#    if RT_THREAD_PRIORITY_MAX > 32
    ...
    local_highest_ready_priority =
        (number << 3) + __rt_ffs(pcpu->ready_table[number]) - 1;
#    else
    ...
    if (highest_ready_priority < local_highest_ready_priority) {
        *highest_prio = highest_ready_priority;
        highest_priority_thread =
            rt_list_entry(rt_thread_priority_table[highest_ready_priority].next,
                          struct rt_thread, tlist);
    } else {
        *highest_prio = local_highest_ready_priority;
        highest_priority_thread =
            rt_list_entry(pcpu->priority_table[local_highest_ready_priority].next,
                          struct rt_thread, tlist);
    }
...
#    endif /* RT_THREAD_PRIORITY_MAX > 32 */
    ...
    highest_priority_thread = rt_list_entry(
        rt_thread_priority_table[highest_ready_priority].next, struct rt_thread, tlist);
...
#    ifdef RT_USING_SIGNALS
    if ((current_thread->stat & RT_THREAD_STAT_MASK) == RT_THREAD_SUSPEND) {
        ...
        if ((current_thread->stat & RT_THREAD_STAT_SIGNAL_MASK) &
    ...
    if (current_thread->scheduler_lock_nest == 1) /* whether lock scheduler */
    {
        ...
        if (rt_thread_ready_priority_group != 0 || pcpu->priority_group != 0) {
            ...
            if ((current_thread->stat & RT_THREAD_STAT_MASK) == RT_THREAD_RUNNING) {
                ...
                } else if (current_thread->current_priority == highest_ready_priority &&
                           (current_thread->stat & RT_THREAD_STAT_YIELD_MASK) == 0) {
                    to_thread = current_thread;
                } else {
            ...
            if (to_thread != current_thread) {
                ...
                rt_schedule_remove_thread(to_thread);
                to_thread->stat =
                    RT_THREAD_RUNNING | (to_thread->stat & ~RT_THREAD_STAT_MASK);
                ...
                RT_DEBUG_LOG(RT_DEBUG_SCHEDULER,
                             ("[%d]switch to priority#%d "
                              ...
                              pcpu->irq_nest, highest_ready_priority, RT_NAME_MAX,
                              to_thread->name, to_thread->sp, RT_NAME_MAX,
                              current_thread->name, current_thread->sp));
    ...
    if (rt_scheduler_lock_nest == 0) {
        ...
        if (rt_thread_ready_priority_group != 0) {
            ...
            if ((rt_current_thread->stat & RT_THREAD_STAT_MASK) == RT_THREAD_RUNNING) {
                ...
                } else if (rt_current_thread->current_priority ==
            ...
            if (to_thread != rt_current_thread) {
                ...
                RT_DEBUG_LOG(RT_DEBUG_SCHEDULER,
                             ("[%d]switch to priority#%d "
                              ...
                              to_thread->name, to_thread->sp, RT_NAME_MAX,
                              from_thread->name, from_thread->sp));
...
#    endif /* RT_USING_SIGNALS */
                ...
                rt_current_thread->stat =
                    RT_THREAD_RUNNING | (rt_current_thread->stat & ~RT_THREAD_STAT_MASK);
            }
        }
    }
...
/**
 * @brief This function checks whether a scheduling is needed after an IRQ context
 * switching. If yes, it will select one thread with the highest priority level, and then
 * switch to it.
 */
#ifdef RT_USING_SMP
...
#    endif /* RT_THREAD_PRIORITY_MAX > 32 */
        ...
        /* there are some time slices left, inserting thread after ready list to schedule
           it firstly at next time*/
        else {
...
#    endif /* RT_THREAD_PRIORITY_MAX > 32 */
        ...
        if ((thread->stat & RT_THREAD_STAT_YIELD_MASK) != 0) {
            rt_list_insert_before(
                &(rt_cpu_index(bind_cpu)->priority_table[thread->current_priority]),
                &(thread->tlist));
        }
        /* there are some time slices left, inserting thread after ready list to schedule
           it firstly at next time*/
...
void rt_schedule_insert_thread(struct rt_thread * thread) {
    ...
    /* there are some time slices left, inserting thread after ready list to schedule it
       firstly at next time*/
    else {
        rt_list_insert_after(&(rt_thread_priority_table[thread->current_priority]),
                             &(thread->tlist));
    }
