 ...
 */
rt_size_t
rt_device_write(rt_device_t dev, rt_off_t pos, const void * buffer, rt_size_t size) {
    /* parameter check */
    RT_ASSERT(dev != RT_NULL);
    RT_ASSERT(rt_object_get_type(&dev->parent) == RT_Object_Class_Device);
...
/**
 * @brief This function will set the reception indication callback function. This callback
 * function is invoked when this device receives data.
 *
 * @param dev is the pointer of device driver structure.
 *
