...
int msh_exec_script(const char * cmd_line, int size) {
    ...
    while ((cmd_line[cmd_length] != ' ' && cmd_line[cmd_length] != '\t') &&
           cmd_length < size)
        cmd_length++;
    ...
    if (argv[1][0] == '-') {
        for (n = 0; argv[1][n]; n++) {
            switch (argv[1][n]) {
                ...
                case 'v': v = 1; break;
                case '-': break;
                default: rt_kprintf("Error: Bad option: %c\n", argv[1][n]); return 0;
            }
        }
        argc -= 1;
        argv = argv + 1;
...
static int                   cmd_mount(int argc, char ** argv) {
    if (argc == 1) {
        ...
        rt_kprintf("----------  ------  ----------\n");
        for (iter = &filesystem_table[0]; iter < &filesystem_table[DFS_FILESYSTEMS_MAX];
             iter++) {
            if ((iter != NULL) && (iter->path != NULL)) {
                rt_kprintf("%-10s  %-6s  %-s\n", iter->ops->name,
                                             iter->dev_id->parent.name, iter->path);
            }
        }
        return 0;
    } else if (argc == 4) {
