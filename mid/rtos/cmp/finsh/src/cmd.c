 ...
 * 2018-12-27     Jesven       Fix the problem that disable interrupt too long in
 * list_thread Provide protection for the "first layer of objects" when list_* 2020-04-07
 * chenhui      add clear 2022-07-02     Stanley Lwin add list command
 */
...
static void
list_find_init(list_get_next_t * p, rt_uint8_t type, rt_list_t ** array, int nr) {
...
#    ifdef RT_USING_SMP
    rt_kprintf(
        "%-*.s cpu bind pri  status      sp     stack size max used left tick  error\n",
        maxlen, item_title);
    object_split(maxlen);
    rt_kprintf(" --- ---- ---  ------- ---------- ----------  ------  ---------- ---\n");
#    else
...
#    ifdef RT_USING_SMP
                    if (thread->oncpu != RT_CPU_DETACHED)
                        rt_kprintf("%-*.*s %3d %3d %4d ", maxlen, RT_NAME_MAX,
                                   thread->name, thread->oncpu, thread->bind_cpu,
                                   thread->current_priority);
                    else
                        rt_kprintf("%-*.*s N/A %3d %4d ", maxlen, RT_NAME_MAX,
                                   thread->name, thread->bind_cpu,
                                   thread->current_priority);

#    else
                    rt_kprintf("%-*.*s %3d ", maxlen, RT_NAME_MAX, thread->name,
                               thread->current_priority);
#    endif /*RT_USING_SMP*/
...
#    if defined(ARCH_CPU_STACK_GROWS_UPWARD)
                    ...
                    rt_kprintf(" 0x%08x 0x%08x    %02d%%   0x%08x %03d\n",
                               ...
                               thread->stack_size,
                               ((rt_ubase_t)ptr - (rt_ubase_t)thread->stack_addr) * 100 /
                                   thread->stack_size,
...
#    else
                    ...
                    rt_kprintf(" 0x%08x 0x%08x    %02d%%   0x%08x %s\n",
                               ...
                               thread->remaining_tick, rt_strerror(thread->error));
#    endif
                }
            }
        }
...
long list_sem(void) {
    ...
    list_find_init(&find_arg, RT_Object_Class_Semaphore, obj_list,
                   sizeof(obj_list) / sizeof(obj_list[0]));
        ...
        next = list_get_next(next, &find_arg);
        {
            ...
            for (i = 0; i < find_arg.nr_out; i++) {
                ...
                if (!rt_list_isempty(&sem->parent.suspend_thread)) {
                    rt_kprintf("%-*.*s %03d %d:", maxlen, RT_NAME_MAX,
...
long list_event(void) {
    ...
    list_find_init(&find_arg, RT_Object_Class_Event, obj_list,
                   sizeof(obj_list) / sizeof(obj_list[0]));
        ...
        next = list_get_next(next, &find_arg);
        {
            ...
            for (i = 0; i < find_arg.nr_out; i++) {
                ...
                if (!rt_list_isempty(&e->parent.suspend_thread)) {
                    rt_kprintf("%-*.*s  0x%08x %03d:", maxlen, RT_NAME_MAX,
...
long list_mutex(void) {
    ...
    list_find_init(&find_arg, RT_Object_Class_Mutex, obj_list,
                   sizeof(obj_list) / sizeof(obj_list[0]));
...
long list_mailbox(void) {
    ...
    list_find_init(&find_arg, RT_Object_Class_MailBox, obj_list,
                   sizeof(obj_list) / sizeof(obj_list[0]));
        ...
        next = list_get_next(next, &find_arg);
        {
            ...
            for (i = 0; i < find_arg.nr_out; i++) {
                ...
                if (!rt_list_isempty(&m->parent.suspend_thread)) {
                    rt_kprintf("%-*.*s %04d  %04d %d:", maxlen, RT_NAME_MAX,
                               m->parent.parent.name, m->entry, m->size,
                               rt_list_len(&m->parent.suspend_thread));
...
long list_msgqueue(void) {
    ...
    list_find_init(&find_arg, RT_Object_Class_MessageQueue, obj_list,
                   sizeof(obj_list) / sizeof(obj_list[0]));
    ...
    do {
        next = list_get_next(next, &find_arg);
        {
            ...
            for (i = 0; i < find_arg.nr_out; i++) {
                ...
                if (!rt_list_isempty(&m->parent.suspend_thread)) {
                    rt_kprintf("%-*.*s %04d  %d:", maxlen, RT_NAME_MAX,
                               m->parent.parent.name, m->entry,
                               rt_list_len(&m->parent.suspend_thread));
                    show_wait_queue(&(m->parent.suspend_thread));
                    rt_kprintf("\n");
                } else {
        ...
        next = list_get_next(next, &find_arg);
        {
            ...
            for (i = 0; i < find_arg.nr_out; i++) {
                ...
                rt_kprintf("%-*.*s %-010d %-013d %-05d\n", maxlen, RT_NAME_MAX,
                           mh->parent.name, mh->pool_size, mh->max_used_size,
                           mh->available_size);
            }
        }
    } while (next != (rt_list_t *)RT_NULL);
...
long list_mempool(void) {
    ...
    list_find_init(&find_arg, RT_Object_Class_MemPool, obj_list,
                   sizeof(obj_list) / sizeof(obj_list[0]));
        ...
        next = list_get_next(next, &find_arg);
        {
            ...
            for (i = 0; i < find_arg.nr_out; i++) {
                ...
                if (suspend_thread_count > 0) {
                    rt_kprintf("%-*.*s %04d  %04d  %04d %d:", maxlen, RT_NAME_MAX,
                               mp->parent.name, mp->block_size, mp->block_total_count,
                               mp->block_free_count, suspend_thread_count);
...
long list_timer(void) {
    ...
    list_find_init(&find_arg, RT_Object_Class_Timer, obj_list,
                   sizeof(obj_list) / sizeof(obj_list[0]));
    ...
    do {
        next = list_get_next(next, &find_arg);
        {
            ...
            for (i = 0; i < find_arg.nr_out; i++) {
                ...
                rt_kprintf("%-*.*s 0x%08x 0x%08x ", maxlen, RT_NAME_MAX,
...
long list_device(void) {
    ...
    list_find_init(&find_arg, RT_Object_Class_Device, obj_list,
                   sizeof(obj_list) / sizeof(obj_list[0]));
    ...
    do {
        next = list_get_next(next, &find_arg);
        {
            ...
            for (i = 0; i < find_arg.nr_out; i++) {
                ...
                rt_kprintf("%-*.*s %-20s %-8d\n", maxlen, RT_NAME_MAX,
                           device->parent.name, device_type, device->ref_count);
            }
        }
    } while (next != (rt_list_t *)RT_NULL);
