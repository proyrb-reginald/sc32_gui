    ...
    if (shell->line_position != 0) {
        ...
        if (shell->history_count >= FINSH_HISTORY_LINES) {
            ...
            if (memcmp(&shell->cmd_history[FINSH_HISTORY_LINES - 1], shell->line,
                ...
                rt_memcpy(&shell->cmd_history[index][0], shell->line,
                          shell->line_position);
            ...
            if (shell->history_count == 0 ||
                ...
                rt_memset(&shell->cmd_history[shell->history_count][0], 0,
                          FINSH_CMD_SIZE);
                rt_memcpy(&shell->cmd_history[shell->history_count][0], shell->line,
                          shell->line_position);
...
__declspec(allocate("FSymTab$a"))
const struct finsh_syscall __fsym_begin = {__fsym_begin_name, __fsym_begin_desc, NULL};

#        pragma section("FSymTab$z", read)
const char                 __fsym_end_name[] = "__end";
const char                 __fsym_end_desc[] = "end of finsh";
__declspec(allocate("FSymTab$z"))
const struct finsh_syscall __fsym_end = {__fsym_end_name, __fsym_end_desc, NULL};
#    endif
...
#    else
    shell  = &_shell;
    tid    = &finsh_thread;
    result = rt_thread_init(&finsh_thread, FINSH_THREAD_NAME, finsh_thread_entry, RT_NULL,
