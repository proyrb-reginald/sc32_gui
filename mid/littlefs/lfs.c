...
static int lfs_bd_read(lfs_t *             lfs,
    ...
    while (size > 0) {
        ...
        rcache->size  = lfs_min(
            lfs_min(lfs_alignup(off + hint, lfs->cfg->read_size), lfs->cfg->block_size) -
                rcache->off,
            lfs->cfg->cache_size);
        int err = lfs->cfg->read(lfs->cfg, rcache->block, rcache->off, rcache->buffer,
                                 rcache->size);
        LFS_ASSERT(err <= 0);
        if (err) {
            return err;
...
#ifndef LFS_READONLY
static int
lfs_bd_flush(lfs_t * lfs, lfs_cache_t * pcache, lfs_cache_t * rcache, bool validate) {
...
#ifndef LFS_READONLY
static int
lfs_bd_sync(lfs_t * lfs, lfs_cache_t * pcache, lfs_cache_t * rcache, bool validate) {
    lfs_cache_drop(lfs, rcache);
...
static inline bool lfs_pair_issync(const lfs_block_t paira[2],
...
static lfs_stag_t
           lfs_fs_parent(lfs_t * lfs, const lfs_block_t dir[2], lfs_mdir_t * parent);
...
static lfs_ssize_t
           lfs_file_read_(lfs_t * lfs, lfs_file_t * file, void * buffer, lfs_size_t size);
...
static int lfs_alloc_lookahead(void * p, lfs_block_t block) {
    lfs_t *     lfs = (lfs_t *)p;
    lfs_block_t off =
        ((block - lfs->lookahead.start) + lfs->block_count) % lfs->block_count;
...
static lfs_stag_t lfs_dir_getslice(lfs_t *            lfs,
    ...
    while (off >= sizeof(lfs_tag_t) + lfs_tag_dsize(ntag)) {
        ...
        lfs_tag_t tag = ntag;
        int err = lfs_bd_read(lfs, NULL, &lfs->rcache, sizeof(ntag), dir->pair[0], off,
                              &ntag, sizeof(ntag));
        if (err) {
            return err;
        }
...
static int lfs_dir_getread(lfs_t *             lfs,
    ...
    while (size > 0) {
        ...
        rcache->off   = lfs_aligndown(off, lfs->cfg->read_size);
        rcache->size =
            lfs_min(lfs_alignup(off + hint, lfs->cfg->read_size), lfs->cfg->cache_size);
        int err = lfs_dir_getslice(lfs, dir, gmask, gtag, rcache->off, rcache->buffer,
                                   rcache->size);
        if (err < 0) {
            return err;
        }
    ...
    while (true) {
        {
            if (off + lfs_tag_dsize(ptag) < dir->off) {
                ...
                int err = lfs_bd_read(lfs, NULL, &lfs->rcache, sizeof(tag), dir->pair[0],
                                      off, &tag, sizeof(tag));
                if (err) {
                    return err;
                }
    ...
    popped:
        ...
        if (lfs_tag_type3(tag) == LFS_FROM_NOOP) {
            ...
            for (unsigned i = 0; i < lfs_tag_size(tag); i++) {
                ...
                res                       = cb(data,
                                               LFS_MKTAG(LFS_TYPE_USERATTR + a[i].type, lfs_tag_id(tag) + diff,
                                                         a[i].size),
                                               a[i].buffer);
                if (res < 0) {
                    return res;
...
lfs_dir_fetchmatch(lfs_t *           lfs,
    ...
    for (int i = 0; i < 2; i++) {
        ...
        while (true) {
            ...
            if (lfs_tag_type1(tag) == LFS_TYPE_NAME) {
                ...
                err = lfs_bd_read(lfs, NULL, &lfs->rcache, lfs->cfg->block_size,
                                  dir->pair[0], off + sizeof(tag), &fcrc, sizeof(fcrc));
                if (err) {
                    if (err == LFS_ERR_CORRUPT) {
                        break;
                    }
            ...
            if ((fmask & tag) == (fmask & ftag)) {
                int res =
                    cb(data, tag, &(struct lfs_diskoff){dir->pair[0], off + sizeof(tag)});
                if (res < 0) {
        ...
        if (maybeerased && dir->off % lfs->cfg->prog_size == 0) {
            ...
            if (lfs_fs_disk_version(lfs) < 0x00020001) {
                ...
                int      err   = lfs_bd_crc(lfs, NULL, &lfs->rcache, lfs->cfg->block_size,
    ...
    LFS_ERROR("Corrupted dir pair at {0x%" PRIx32 ", 0x%" PRIx32 "}", dir->pair[0],
              dir->pair[1]);
    return LFS_ERR_CORRUPT;
}
...
static int lfs_dir_fetch(lfs_t * lfs, lfs_mdir_t * dir, const lfs_block_t pair[2]) {
    ...
    return (int)lfs_dir_fetchmatch(lfs, dir, pair, (lfs_tag_t)-1, (lfs_tag_t)-1, NULL,
                                   NULL, NULL);
}
...
static int
lfs_dir_getinfo(lfs_t * lfs, lfs_mdir_t * dir, uint16_t id, struct lfs_info * info) {
    if (id == 0x3ff) {
...
static lfs_stag_t
lfs_dir_find(lfs_t * lfs, lfs_mdir_t * dir, const char ** path, uint16_t * id) {
        ...
        *path = name;
        ...
        while (true) {
            tag = lfs_dir_fetchmatch(lfs, dir, dir->tail, LFS_MKTAG(0x780, 0, 0),
                                     LFS_MKTAG(LFS_TYPE_NAME, 0, namelen), id,
                                     lfs_dir_find_match,
...
static int lfs_dir_commitcrc(lfs_t * lfs, struct lfs_commit * commit) {
    ...
    const lfs_off_t end =
        lfs_alignup(lfs_min(commit->off + 5 * sizeof(uint32_t), lfs->cfg->block_size),
                    lfs->cfg->prog_size);
    ...
    while (commit->off < end) {
        ...
        if (noff >= end && noff <= lfs->cfg->block_size - lfs->cfg->prog_size) {
            ...
            int err = lfs_bd_read(lfs, NULL, &lfs->rcache, lfs->cfg->prog_size,
...
#    endif
            {
                ...
                err = lfs_bd_crc(lfs, NULL, &lfs->rcache, lfs->cfg->prog_size,
                ...
                err = lfs_dir_commitattr(
                    lfs, commit, LFS_MKTAG(LFS_TYPE_FCRC, 0x3ff, sizeof(struct lfs_fcrc)),
                    &fcrc);
                if (err) {
                    return err;
                }
    ...
    int err = lfs_bd_crc(lfs, NULL, &lfs->rcache, off1 + sizeof(uint32_t), commit->block,
                         off, off1 - off, &crc);
    if (err) {
        return err;
    }
...
static int lfs_dir_alloc(lfs_t * lfs, lfs_mdir_t * dir) {
    ...
    int err  = lfs_bd_read(lfs, NULL, &lfs->rcache, sizeof(dir->rev), dir->pair[0], 0,
...
static int lfs_dir_drop(lfs_t * lfs, lfs_mdir_t * dir, lfs_mdir_t * tail) {
    ...
    err = lfs_dir_commit(
        lfs, dir,
        LFS_MKATTRS({LFS_MKTAG(LFS_TYPE_TAIL + tail->split, 0x3ff, 8), tail->tail}));
    ...
    while (true) {
        {
            ...
            err = lfs_dir_traverse(
                lfs, source, 0, 0xffffffff, attrs, attrcount, LFS_MKTAG(0x400, 0x3ff, 0),
                LFS_MKTAG(LFS_TYPE_NAME, 0, 0), begin, end, -begin, lfs_dir_commit_commit,
    ...
    while (true) {
        ...
        while (end - split > 1) {
            ...
            int err = lfs_dir_traverse(lfs, source, 0, 0xffffffff, attrs, attrcount,
                                       ...
                                       LFS_MKTAG(LFS_TYPE_NAME, 0, 0), split, end, -split,
        ...
        if (err) {
            ...
            // performance
            LFS_WARN("Unable to split {0x%" PRIx32 ", 0x%" PRIx32 "}", dir->pair[0],
                     dir->pair[1]);
            break;
        } else {
            end = split;
...
static int lfs_dir_relocatingcommit(lfs_t *                  lfs,
    ...
    if (dir->erased) {
        ...
        lfs_pair_tole32(dir->tail);
        int err = lfs_dir_traverse(lfs, dir, dir->off, dir->etag, attrs, attrcount, 0, 0,
                                   0, 0, 0, lfs_dir_commit_commit,
                                   &(struct lfs_dir_commit_commit){lfs, &commit});
        lfs_pair_fromle32(dir->tail);
        if (err) {
...
static int lfs_mkdir_(lfs_t * lfs, const char * path) {
    ...
    err = lfs_dir_commit(
        lfs, &dir, LFS_MKATTRS({LFS_MKTAG(LFS_TYPE_SOFTTAIL, 0x3ff, 8), pred.tail}));
    ...
    if (cwd.m.split) {
        ...
        err = lfs_dir_commit(
            lfs, &pred, LFS_MKATTRS({LFS_MKTAG(LFS_TYPE_SOFTTAIL, 0x3ff, 8), dir.pair}));
...
static int lfs_ctz_find(lfs_t *             lfs,
    ...
    while (current > target) {
        ...
        int err = lfs_bd_read(lfs, pcache, rcache, sizeof(head), head, 4 * skip, &head,
                              sizeof(head));
...
static int lfs_file_opencfg_(lfs_t *                        lfs,
    ...
    if (tag == LFS_ERR_NOENT) {
        ...
        // it may happen that the file name doesn't fit in the metadata blocks, e.g., a
        // 256 byte file name will not fit in a 128 byte block.
    ...
    if (lfs_tag_type3(tag) == LFS_TYPE_INLINESTRUCT) {
        ...
        if (file->ctz.size > 0) {
            lfs_stag_t res = lfs_dir_get(
                lfs, &file->m, LFS_MKTAG(0x700, 0x3ff, 0),
...
lfs_file_flushedread(lfs_t * lfs, lfs_file_t * file, void * buffer, lfs_size_t size) {
    ...
    while (nsize > 0) {
        ...
        if (file->flags & LFS_F_INLINE) {
            int err = lfs_dir_getread(lfs, &file->m, NULL, &file->cache,
                                      lfs->cfg->block_size, LFS_MKTAG(0xfff, 0x1ff, 0),
...
static lfs_soff_t
lfs_file_seek_(lfs_t * lfs, lfs_file_t * file, lfs_soff_t off, int whence) {
    // find new pos
    //
...
static int lfs_remove_(lfs_t * lfs, const char * path) {
    ...
    // delete the entry
    err = lfs_dir_commit(
        lfs, &cwd, LFS_MKATTRS({LFS_MKTAG(LFS_TYPE_DELETE, lfs_tag_id(tag), 0), NULL}));
    if (err) {
        lfs->mlist = dir.next;
        return err;
    }
...
static int lfs_commitattr(lfs_t *      lfs,
    ...
    return lfs_dir_commit(
        lfs, &cwd, LFS_MKATTRS({LFS_MKTAG(LFS_TYPE_USERATTR + type, id, size), buffer}));
}
#endif
...
static int lfs_init(lfs_t * lfs, const struct lfs_config * cfg) {
    ...
    LFS_ASSERT(
        lfs->cfg->inline_max == (lfs_size_t)-1 ||
        lfs->cfg->inline_max <=
            ((lfs->cfg->metadata_max) ? lfs->cfg->metadata_max : lfs->cfg->block_size) /
                8);
...
int lfs_fs_traverse_(lfs_t * lfs,
    ...
    for (lfs_file_t * f = (lfs_file_t *)lfs->mlist; f; f = f->next) {
        ...
        if ((f->flags & LFS_F_WRITING) && !(f->flags & LFS_F_INLINE)) {
            int err = lfs_ctz_traverse(lfs, &f->cache, &lfs->rcache, f->block, f->pos, cb,
                                       data);
            if (err) {
                return err;
            }
...
#ifndef LFS_READONLY
static lfs_stag_t
lfs_fs_parent(lfs_t * lfs, const lfs_block_t pair[2], lfs_mdir_t * parent) {
...
static void lfs_fs_prepsuperblock(lfs_t * lfs, bool needssuperblock) {
    lfs->gstate.tag =
        (lfs->gstate.tag & ~LFS_MKTAG(0, 0, 0x200)) | (uint32_t)needssuperblock << 9;
}
...
static int lfs_fs_desuperblock(lfs_t * lfs) {
    ...
    LFS_DEBUG("Rewriting superblock {0x%" PRIx32 ", 0x%" PRIx32 "}", lfs->root[0],
              lfs->root[1]);
...
static int lfs_fs_demove(lfs_t * lfs) {
    ...
    lfs_fs_prepmove(lfs, 0x3ff, NULL);
    err = lfs_dir_commit(lfs, &movedir,
                ...
                if (pass == 0 && tag != LFS_ERR_NOENT) {
                    ...
                    if (!lfs_pair_issync(pair, pdir.tail)) {
                        ...
                        state = lfs_dir_orphaningcommit(
                            lfs, &pdir,
                            LFS_MKATTRS({LFS_MKTAG_IF(moveid != 0x3ff, LFS_TYPE_DELETE,
                                                      moveid, 0),
                                         NULL},
...
static int lfs1_bd_read(lfs_t *     lfs,
                        lfs_block_t block,
                        lfs_off_t   off,
                        void *      buffer,
                        lfs_size_t  size) {
...
static int lfs1_bd_crc(lfs_t *     lfs,
                       lfs_block_t block,
                       lfs_off_t   off,
                       lfs_size_t  size,
                       uint32_t *  crc) {
...
static int lfs1_dir_fetch(lfs_t * lfs, lfs1_dir_t * dir, const lfs_block_t pair[2]) {
    ...
    if (!valid) {
        LFS_ERROR("Corrupted dir pair at {0x%" PRIx32 ", 0x%" PRIx32 "}", tpair[0],
                  tpair[1]);
        return LFS_ERR_CORRUPT;
    }
...

    {
        ...
        while (!lfs_pair_isnull(dir1.d.tail)) {
                ...
                err = lfs_dir_commit(
                    ...
                    LFS_MKATTRS({LFS_MKTAG(LFS_TYPE_CREATE, id, 0), NULL},
                                ...
                                {LFS_MKTAG_IF_ELSE(isdir, LFS_TYPE_DIRSTRUCT, id,
                                                   sizeof(entry1.d.u), LFS_TYPE_CTZSTRUCT,
            ...
            for (lfs_off_t i = 0; i < dir2.off; i++) {
                ...
                err = lfs_bd_prog(lfs, &lfs->pcache, &lfs->rcache, true, dir1.head[1], i,
                                  &dat, 1);
                if (err) {
                    goto cleanup;
                }
...
int lfs_format(lfs_t * lfs, const struct lfs_config * cfg) {
    ...
    LFS_TRACE("lfs_format(%p, %p {.context=%p, "
              ...
              cfg->block_size, cfg->block_count, cfg->block_cycles, cfg->cache_size,
              cfg->lookahead_size, cfg->read_buffer, cfg->prog_buffer,
              cfg->lookahead_buffer, cfg->name_max, cfg->file_max, cfg->attr_max);
...
lfs_ssize_t lfs_getattr(lfs_t *      lfs,
    ...
    LFS_TRACE("lfs_getattr(%p, \"%s\", %" PRIu8 ", %p, %" PRIu32 ")", (void *)lfs, path,
              type, buffer, size);
...
int lfs_setattr(lfs_t *      lfs,
    ...
    LFS_TRACE("lfs_setattr(%p, \"%s\", %" PRIu8 ", %p, %" PRIu32 ")", (void *)lfs, path,
              type, buffer, size);
...
lfs_ssize_t
lfs_file_read(lfs_t * lfs, lfs_file_t * file, void * buffer, lfs_size_t size) {
    int err = LFS_LOCK(lfs->cfg);
    if (err) {
        return err;
    }
    LFS_TRACE("lfs_file_read(%p, %p, %p, %" PRIu32 ")", (void *)lfs, (void *)file, buffer,
              size);
    LFS_ASSERT(lfs_mlist_isopen(lfs->mlist, (struct lfs_mlist *)file));
...
lfs_file_write(lfs_t * lfs, lfs_file_t * file, const void * buffer, lfs_size_t size) {
    ...
    LFS_TRACE("lfs_file_write(%p, %p, %p, %" PRIu32 ")", (void *)lfs, (void *)file,
              buffer, size);
    LFS_ASSERT(lfs_mlist_isopen(lfs->mlist, (struct lfs_mlist *)file));
...
lfs_soff_t lfs_file_seek(lfs_t * lfs, lfs_file_t * file, lfs_soff_t off, int whence) {
    ...
    LFS_TRACE("lfs_file_seek(%p, %p, %" PRId32 ", %d)", (void *)lfs, (void *)file, off,
              whence);
    LFS_ASSERT(lfs_mlist_isopen(lfs->mlist, (struct lfs_mlist *)file));
